...
// Асимптотическая сложность умножения Карацубы 3T(n/2) + O(n) ~ O(n^log2(3)) ~ O(n^1.585)

long long int ** mult_matrix(long long int **A, long long int **B, int matrix_size, bool mult_flag) {
    unsigned long long int count_karatsuba_mult_operations = 0, count_native_mult_operations = 0;
    ...
    for (int i = 0; i < matrix_size; i++) {
        for (int j = 0; j < matrix_size; j++) {
            for (int k = 0; k < matrix_size; k++) {
                ...
                if (!mult_flag) {
                    vector_3 = native_mul(vector_1, vector_2, count_native_mult_operations);
                } else {
                    vector_3 = karatsuba_mul(vector_1, vector_2, count_karatsuba_mult_operations);
                }
                finalize(vector_3);
    ...
    if (!mult_flag) {
        std::cout << "count_native_mult_operations: " << count_native_mult_operations << std::endl;
    } else {
        std::cout << "count_karatsuba_mult_operations: " << count_karatsuba_mult_operations << std::endl;
    }
...
std::vector<int> native_mul(const std::vector<int>& vector_1, const std::vector<int>& vector_2,
...
std::vector<int> karatsuba_mul(const std::vector<int>& vector_1, const std::vector<int>& vector_2,
    ...
    std::vector<int> P3 = karatsuba_mul(Xlr, Ylr, count_karatsuba_mult_operations);
